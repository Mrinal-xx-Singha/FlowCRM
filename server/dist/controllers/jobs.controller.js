...
const createJob = async (req, res) => {
    ...
    const trimmedDescription = description !== undefined ? description.trim() : null;
    try {
        const customerQuery = "SELECT id FROM customers WHERE id=$1 AND user_id=$2";
        const customerValues = [customerId, userId];
        const customerResult = await dbConnect_1.pool.query(customerQuery, customerValues);
...
const getJobs = async (req, res) => {
    ...
    try {
        let jobQuery = "SELECT jobs.id, jobs.customer_id, jobs.title, jobs.description,jobs.status,jobs.due_date,jobs.created_at,jobs.updated_at,customers.name AS customer_name FROM jobs JOIN customers ON jobs.customer_id = customers.id WHERE jobs.user_id = $1 ";
...
const getJob = async (req, res) => {
    ...
    try {
        const jobQuery = "SELECT jobs.id, jobs.customer_id, jobs.title, jobs.description, jobs.status, jobs.due_date, jobs.created_at, jobs.updated_at, customers.name AS customer_name FROM jobs JOIN customers ON jobs.customer_id = customers.id WHERE jobs.id = $1 AND jobs.user_id = $2";
