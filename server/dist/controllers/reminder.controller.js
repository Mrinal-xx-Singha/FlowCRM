...
const createReminder = async (req, res) => {
    ...
    try {
        ...
        if (customerId !== null) {
            ...
            const customerResult = await dbConnect_1.pool.query(customerQuery, [
                customerId,
                userId,
            ]);
        ...
        if (jobId !== null) {
            ...
            const jobResult = await dbConnect_1.pool.query(jobQuery, [jobId, userId]);
...
const getReminderById = async (req, res) => {
    ...
    `;
        const reminderResult = await dbConnect_1.pool.query(reminderQuery, [
            reminderId,
            userId,
        ]);
...
const updateReminder = async (req, res) => {
    ...
    try {
        ...
        const checkQuery = "SELECT id FROM reminders WHERE id = $1 AND user_id = $2";
        const checkResult = await dbConnect_1.pool.query(checkQuery, [reminderId, userId]);
      ...
      RETURNING *
    `;
        const updateResult = await dbConnect_1.pool.query(updateQuery, [
            ...values,
            reminderId,
            userId,
        ]);
...
const deleteReminder = async (req, res) => {
    ...
    try {
        const deleteQuery = "DELETE FROM reminders WHERE id = $1 AND user_id = $2 RETURNING *";
        const deleteResult = await dbConnect_1.pool.query(deleteQuery, [reminderId, userId]);
