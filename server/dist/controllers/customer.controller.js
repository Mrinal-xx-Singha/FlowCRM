...
exports.deleteCustomer = exports.updateCustomer = exports.getACustomer = exports.listAllCustomer = exports.createCustomer = void 0;
...
const createCustomer = async (req, res) => {
    try {
        ...
        const customer = await dbConnect_1.pool.query(customerQuery, values);
...
const listAllCustomer = async (req, res) => {
    try {
        ...
        const result = await dbConnect_1.pool.query(customerQuery, [userId]);
...
exports.listAllCustomer = listAllCustomer;
const getACustomer = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
        const result = await dbConnect_1.pool.query("SELECT id,name,email,phone,notes,created_at, updated_at FROM customers WHERE id= $1 AND user_id = $2", [customerId, userId]);
...
const updateCustomer = async (req, res) => {
    ...
    `;
        const result = await dbConnect_1.pool.query(customerQuery, values);
        if (result.rows.length === 0)
            return res.status(404).json({ message: "Customer not found" });
        return res.status(200).json({ customer: result.rows[0] });
    }
...
const deleteCustomer = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
        const result = await dbConnect_1.pool.query("DELETE FROM customers WHERE id=$1 AND user_id =$2 RETURNING id", [customerId, userId]);
        if (result.rows.length === 0)
            return res.status(404).json({ message: "Customer not found" });
