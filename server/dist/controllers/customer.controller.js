...
const getACustomer = async (req, res) => {
    ...
    try {
        const customerQuery = "SELECT id,name,email,phone,notes,created_at, updated_at FROM customers WHERE id= $1 AND user_id = $2";
...
const deleteCustomer = async (req, res) => {
    ...
    try {
        const customerQuery = "DELETE FROM customers WHERE id=$1 AND user_id =$2 RETURNING id";
        const values = [customerId, userId];
        const result = await dbConnect_1.pool.query(customerQuery, values);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "Customer not found" });
